...
/**
 * @brief Called when a path is defined, and the request doesn't 
 * match this path.
 *
 * @param req Full request string.
 *
 * @param req_len Length of the request string.
 *
 * @param res Response buffer, must be initialized and formatted
 * as a HTTP response.
 *
 * @returns Length of the response.
 */
uint64_t onbaserequest(const char *req, uint64_t req_len, char **res) {
    char msg[] = "HTTP/1.1 200 OK\r\nConnection: close\r\n\r\n";

    *res = malloc(sizeof(msg) - 1);
    memcpy(*res, msg, sizeof(msg) - 1);
